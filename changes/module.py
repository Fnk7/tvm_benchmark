    ...
    def export_library(self, file_name, fcompile=None, addons=None, workspace_dir=None, **kwargs):
        ...
        if self.imported_modules:
            if enabled("llvm") and llvm_target_triple:
                ...
                m.save(path_obj)
                files.append(path_obj)
            elif "c_target_triple" in kwargs:
                path_obj = os.path.join(workspace_dir, f"devc.o")
                m = _ffi_api.ModulePackImportsToLLVM(self, is_system_lib, kwargs.pop("c_target_triple"))
                m.save(path_obj)
                files.append(path_obj)
            else:
